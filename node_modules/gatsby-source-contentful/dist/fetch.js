...
function handleContentfulError({
  ...
  reporter.panic({
    id: _report.CODES.GenericContentfulError,
    context: {
...
async function fetchContentTypes({
  ...
  try {
    ...
    try {
      const contentTypeCollection = await pagedGet(client, `getContentTypes`, pageLimit);
      if (contentTypeCollection) {
        reporter.verbose(`Content types fetched ${contentTypeCollection.items.length} (${sourceId})`);
 ...
 */
function pagedGet(client, method, pageLimit, query = {}, skip = 0, aggregatedResponse) {
  if (!client[method]) {
